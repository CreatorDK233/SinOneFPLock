...
typedef struct
{
	...
	UserIdentifyErrorType_t ErrorType;
	ManagerID_t   ManagerID[2];
  ManagerID_t   BackupsManagerID[2];
...
typedef union
{
	struct 
	{
		//低位
		uint8_t Info:1;
		uint8_t StartNetwork:1;
		uint8_t WifiState:1;
		uint8_t PowerLevel:1;
		uint8_t DoorBell:1;
		uint8_t Unlock:1;
		uint8_t RemoteUnlock:1;
		uint8_t Alarm:1;
		
		uint8_t FingerSync:1;
		uint8_t PasswordSync:1;
		uint8_t CardSync:1;
		uint8_t RequestRemoteUnlockPassword:1;
		uint8_t AnswerCmd:1;
		uint8_t SaveKeySuccess:1;
		uint8_t TempPasswordCheck:1;
		uint8_t TempPasswordUnlock:1;
		
		uint8_t DeleteTempPassword:1;
		uint8_t RequestGreenTime:1;
		//高位
	}Bits;
	uint32_t TotalFlagBits;
}WifiPostFlag_t;

//备注:0=指纹试错告警、1=密码试错告警、2=卡试错报告警、3=人脸试错告警、4=假锁（锁舌卡住）、
//5=高温告警、6=超时未关门、7=电子锁舌未弹出、8=防撬告警 、9=钥匙插入、10=低电告警、
//11=电量耗尽告警、12=震动告警、13=布防告警、14=逗留抓拍、15=门铃呼叫、16=错误开锁（连续验证试错告警）
typedef enum
{
	FingerErrorAlarm  	=   0x00,
	PasswordErrorAlarm	=   0x01,
	CardErrorAlarm    	=   0x02,
	FaceErrorAlarm    	=   0x03,
	TongueBadAlarm    	=   0x04,
	TooHotAlarm       	=  	0x05,
	UnclosedOvertime  	=		0x06,
	TongueNotOut      	=		0x07,
	PryAlarm         		=		0x08,
	KeyIn         			=		0x09,
	LowPowerAlarm     	=  	0x0a,
	EmptyPowerAlarm   	=   0x0b,
	ShockAlarm        	= 	0x0c,
	DefenseAlarm      	=   0x0d,
	StayAlarm						=		0x0e,
	DoorBellAlarm     	=   0x0f,
...
typedef struct
{
	WifiPostFlag_t	Flag;//标记待发送和正在发送的数据
	WifiPostFlag_t	SendingFlag;//标记当前正在发送的数据，用于重发判定
	int16_t WifiPostIntervalCnt;
	uint16_t OverTime;
	WifiAlarmType_t AlarmType;
	uint16_t RemoteUnlockCountdown;
}WifiPostMgr_t;
...
typedef struct
{
	...
	uint8_t Passward[8];
	int16_t RequestPasswordTimeCnt;
	bool_t WaitRequestRemoteUnlock;
	bool_t RequestRemoteUnlockisReady;
}RemoteUnlockMgr_t;
...
typedef struct
{
	...
	uint8_t DecryptingData[16];
	uint8_t GreenTime[6];
	int16_t TempPswdCnt;
}OfflineTempPassword_t;
...
typedef struct
{
	...
	OfflineTempPassword_t  OfflineTempPassword;
