...
void RegisterFp(uint16_t UserID)//注册指纹
{
	...
	else if ( FpRegisterMgr.Status == SendStoreCharCmd )
	{
		...
		FpmAckMgr.Status = WaitACK;
		FPM_SendStoreCharCmd(0x01,UserID);		//BuffID is 0x01 ,Page ID is UserID
		FpRegisterMgr.TimeCnt = Def_FPMcmdTimeOutDelay; 
		DEBUG_MARK;
	}
...
void FpUserIdentify(void)//指纹用户认证
{
...
void CheckIfFpUserIsRegistered(uint16_t USERID)//检查指纹用户是否注册
{
...
{
	uint8_t OffsetByte,OffsetBit,ByteValue;
