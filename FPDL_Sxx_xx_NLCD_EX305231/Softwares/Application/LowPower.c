...
void AntiMisAwakeup(uint8_t KeyValue)
{
	if( KeyWakeUpKeyWaitTime <= 0 )
	{
		if(LastWakeUpKeyValue==KeyValue)
		{
				...
				MX_QuitLowPowerMode();
				SystemPowerMgr.AwakeSource = KeyBoardTouch;
				#if defined ProjectIs_BarLock_S6904	|| defined ProjectIs_BarLock_S6904v2	\
				 || defined ProjectIs_BarLock_S4914 || defined ProjectIs_BarLock_S9022
					if( KeyValue == 0x00 )
					{
...
void System_PowerDown(void)
{
  uint16_t i;
	...
	while(1)
	{
	 ...
	 || defined ProjectIs_BarLock_S9022
		LEDsCtrlSwitch = 0;
	#endif
