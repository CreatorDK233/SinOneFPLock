...
//	code uint8_t DuplicateFpStrEn[]={"<Duplicate Fp>"};
	...
	if ( FpUserRegisterMgr.Status == StartFpUserRegister )
	{
		if ( CheckMemoryMgr.FpMasterNum < DEF_MAX_FPMASTER )
		{
			...
			FpUserRegisterMgr.TimeCnt = 0;
			#ifdef Function_Enable_ID_Del
			FpUserRegisterMgr.UserID = Get_Availabe_FPmasterID();
			ReportRegisterID(FPUSER, FpUserRegisterMgr.UserID);
			FpUserRegisterMgr.TimeCnt = Def_GuiTimeDelayCnt4s;
			#endif
		}
		else
		{
...
{
	#ifdef Function_Enable_ID_Del
	uint8_t voiceDelFailPleaseInput[] = {VOICE_Delete, VOICE_Fail, VOICE_PleaseInputID,DEF_VoiceSegmentEndFlag};
	if ( FpUserDeleteMgr.Status == StartFpUserDelete )
	{
		FpUserDeleteMgr.Status = InputUserID;
		DataInputMgr.Status = InputIdle;
	...
	if ( FpUserDeleteMgr.Status == InputUserID )
	{
			...
			{
				FpUserDeleteMgr.UserID = DataInputMgr.Value;
				if ((FpUserDeleteMgr.UserID >= 1) && (FpUserDeleteMgr.UserID <= (DEF_MAX_FPMASTER + DEF_MAX_FPUSER)))
				{
						ReportRegisterID(FPUSER, FpUserDeleteMgr.UserID);
						FpUserDeleteMgr.TimeCnt = 4 * 64;
						FpUserDeleteMgr.Status = ReportUserID;
						DataInputMgr.Status = InputIdle;
				}
				else
				{
						DataInputMgr.Status = InputIdle; // Reinput
						PLAY_VOICE_TWOSEGMENT(VOICE_InputError, VOICE_PleaseInputID);
				}
			}
			else if  (DataInputMgr.Status == InputExit)
			{
	...
	else if (FpUserDeleteMgr.Status == ReportUserID)
	{
		if (FpUserDeleteMgr.TimeCnt-- < 1)
		{
			if (FpUserDeleteMgr.UserID <= 10 )
			{
				FpUserDeleteMgr.Status = InputUserID;
				DataInputMgr.Status = InputIdle;
				PLAY_VOICE_MULTISEGMENTS(voiceDelFailPleaseInput);
			}
			else
			{
				FpUserDeleteMgr.Status = CheckIfFpUserIDisBeUsed;
...
//		DisOneDigital16x8(6,40,DEF_MAX_CARDUSER%10,NormalDisplay);
		...
		if (DataInputMgr.Status == InputEnd)
		{
			...
			{
				ReportRegisterID(CARDUSER, CardUserDeleteMgr.UserID);
				CardUserDeleteMgr.TimeCnt = 4 * 64;
	...
	else if (CardUserDeleteMgr.Status == ReportCardUserID)
	{
		...
		{
			if (IfCardUserIDisRegistered(CardUserDeleteMgr.UserID) == bTRUE)
			{
...
void ShowRegisterPasscodeUser(void)//注册密码用户
{
	...
	if (PasscodeUserRegisterMgr.Status == StartPasscodeUserRegister)
	{
		...
		else
		{
			...
			ReportRegisterID(PASSCODEUSER,PasscodeUserRegisterMgr.UserID);
			PasscodeUserRegisterMgr.TimeCnt = Def_GuiTimeDelayCnt4s;
			#endif
		}
	}
...
void ShowDeletePasscodeUser(void)//界面-删除密码用户
{
	...
	else if ( PasscodeUserDeleteMgr.Status == InputPasscodeUserID )
	{
		...
		if (DataInputMgr.Status == InputEnd)
		{
			...
			if (PasscodeUserDeleteMgr.UserID >= 1 &&
					PasscodeUserDeleteMgr.UserID <= DEF_MAX_PASSCODEMASTER + DEF_MAX_PASSCODEUSER)
			{
					PasscodeUserDeleteMgr.Status = ReportPasscodeUserID;
					PasscodeUserDeleteMgr.TimeCnt = 4 * 64;
					ReportRegisterID(PASSCODEUSER, PasscodeUserDeleteMgr.UserID);
					DataInputMgr.Status = InputIdle;
			}
			else
			{
	...
	else if (PasscodeUserDeleteMgr.Status == ReportPasscodeUserID)
	{
		if (PasscodeUserDeleteMgr.TimeCnt-- < 1)
		{
			if (PasscodeUserDeleteMgr.UserID <= 2)
			{
					FpUserDeleteMgr.Status = InputUserID;
					DataInputMgr.Status = InputIdle;
					PLAY_VOICE_MULTISEGMENTS(voiceDelFailPleaseInput);
